...
void HierarchicalAllocatorProcess::__allocate()
{
 ...
 Resources allocatedStage2;
  double temp=100000.0,sn_i=100000.0;
  SlaveID someID;
  string s,f_name,frame_name;
  foreach(const SlaveID& slaveId, slaveIds){
    foreach (const string& role, roleSorter->sort()) {
      ...
      s = frameworkSorter->psdsfFrameworkSlaveSelect(slaveId);
     ...
     foreach (const string& token, strings::tokenize(s, ";")) {
          ...
          if(counter == 0)
          {
            //matlab framework ka naam
            frame_name = token;
          }

          if(counter == 1)
          {
            //matlab share ki value
            share_val = token;
          }
        counter++;
      }
  ...
  temp = std::stod (share_val,&sz);
  LOG (INFO) << "\n\nValue of share_val = " <<share_val << "\n\n";
  LOG (INFO) <<"\n\n value of share = " <<temp<<"\n\n";
  }
  if(temp < sn_i)
  {
...
LOG (INFO) << "\n\nFinal min value of ratio : "<<sn_i <<"\n\n";

vector<SlaveID> selected;
selected.push_back(someID);
            //role = "*";
  // At this point resources for quotas are allocated or accounted for.
  // Proceed with allocating the remaining free pool.
  foreach (const SlaveID& slaveId, selected) {
    ...
    foreach (const string& role, roleSorter->sort()) {
      ...
      // change sort() to sortPerSlave(slaveId)
        ...
        LOG (INFO) << "\nSlave Selected " << slaveId << "\nFramework Selected " << frameworkId;
        ...
        // This addition is to implement FINE GRAINED ALLOCATION Policy
        // Let's try this man !!
        ...
        frameworkSorter->add(slaveId, resources);
        frameworkSorter->allocated(f_name, slaveId, resources);
        roleSorter->allocated(role, slaveId, resources);
