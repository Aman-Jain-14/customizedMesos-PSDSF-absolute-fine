...
//Added by Aman
string DRFSorter::psdsfFrameworkSlaveSelect(const SlaveID& slaveId){

  LOG (INFO) << "\n\nEnter PSDSF Slave and Framework Pair Selection function\n\n";
  double sn_i = 10000000.0, temp;
  string frame_t;
  string slave_t;

    foreach(Client client, clients){

      temp = calculateMetric(slaveId, client.name);
      LOG(INFO) <<"\n\nTemp = " << temp ;
      if(temp < sn_i){
          sn_i = temp;
          frame_t = client.name;
  ...
  string final = frame_t + ";" + stringify(sn_i);
  LOG (INFO) << "\n\nValue of sn_i from function : "<<sn_i<<"\n\n";
  LOG (INFO) << "\n\nFINAL String : " << final <<"\n\n";
  return final;
}
